...
def parse_file(filename):
    ...
    for k,v in data_delay_sink_dist.iteritems():
        print k, sorted(v)[int(math.ceil(len(v)*0.9))]

    for k,v in delay_type_percentile.iteritems():
...
if __name__ == "__main__":
    ...
    nodes_v = (9, 16, 25, 36, 49, 64, 81, 100, 169, 256)
            ...
            for nd in nd_possibilities:
                for datarate in datarates:
                    ...
                    for i in range(MIN_ITER, MAX_ITER+1):
                        filename = "cooja_comcom_n" + repr(nnodes) + "_top" + topo + "_nd" + nd + "_l" + repr(datarate) + "_i" + repr(i) + '.txt'
                        filename += 'preproc'
                        try:
    ...
    for scenario in sorted(results_avg[i_metric].keys()):
        ...
        plt.bar(x + bar_width*(i + i/len(colors)/2.0), y, bar_width, yerr=desv, label=pretty_scenario(scenario), color=colors[i%len(colors)], hatch=hatches[(i/len(colors))%len(hatches)], ecolor='black' )
        i+=1
        # print x
        # print y
    ...
    for scenario in sorted(results_avg[i_metric].keys()):
        ...
        print x + bar_width*i, y, bar_width
        plt.bar(x + bar_width*(i + i/len(colors)), y, bar_width, yerr=desv, label=pretty_scenario(scenario), color=colors[i%len(colors)], hatch=hatches[(i/len(colors))%len(hatches)], ecolor='black' )
        i+=1
        # print x
        # print y
    # plt.errorbar(x, y, e, ls=lss[rc], color=colors[rc], marker=markers[rc])
